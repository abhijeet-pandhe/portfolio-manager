...
async function fetchReturns(symbol) {
  ...
  if (valid.length < 4) return { ret12m: 0, ret6m: 0, ret3m: 0 };
...
/**
 * Ranks all Nifty 50 symbols by: 20% × 12M return + 30% × 6M return + 50% × 3M return.
...
async function calculateRankings(symbols) {
  ...
  for (let i = 0; i < symbols.length; i++) {
    ...
    try {
      const { ret12m, ret6m, ret3m, priceLTP, price12m, price6m, price3m } = await withRetry(() => fetchReturns(symbol));
      const score = 0.2 * ret12m + 0.3 * ret6m + 0.5 * ret3m;
      results.push({ symbol, ret12m, ret6m, ret3m, score, priceLTP, price12m, price6m, price3m });
      console.log(formatLine(symbol, priceLTP, price12m, ret12m, price6m, ret6m, price3m, ret3m, score));
    } catch (err) {
      console.log(`  ${symbol.padEnd(15)} ERROR: ${cleanYahooError(err)}`);
      results.push({ symbol, ret12m: 0, ret6m: 0, ret3m: 0, score: 0, failed: true });
    }
...
function formatLine(symbol, ltp, p12m, r12m, p6m, r6m, p3m, r3m, score) {
